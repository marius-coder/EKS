...
class LadeController(LadeController_Personen):
	...
	def __init__(self, AutoDaten, distMinLadung, PersonenDaten, infoLehrpersonal= None,infoGewerbepersonal= None):
		...
		self.Außenstehende = InitZureisende(AutoDaten= AutoDaten,maxLadung= infoLehrpersonal["Ladung"], percent= infoLehrpersonal["Prozent Mitmachende"]
										, anzAutos= infoLehrpersonal["Anzahl"], bLehrer= True)[0]
		self.Außenstehende.extend(InitZureisende(AutoDaten= AutoDaten,maxLadung= infoGewerbepersonal["Ladung"], percent= infoGewerbepersonal["Prozent Mitmachende"]
